...
const RemoveMembers = () => {
  ...
  const searchGroupMembers = userList => {
    ...
    const limit = 30;
    const groupMemberRequestBuilder = new cometChat.GroupMembersRequestBuilder(
      GUID,
    ).setLimit(limit);
    const groupMemberRequest = keyword
      ? groupMemberRequestBuilder.setSearchKeyword(keyword).build()
      : groupMemberRequestBuilder.build();
    ...
    if (shouldRemoveMember(selectedUser)) {
      ...
      cometChat.kickGroupMember(GUID, UID).then(
        response => {
          ...
          showMessage(
            'Info',
            `${selectedUser.name} was removed from the group successfully`,
          );
          searchGroupMembers();
        },
        error => {},
  ...
  const selectItem = item => () => {
    ...
    Alert.alert('Confirm', `Do you want remove ${item.name} from the group?`, [
      {
        text: 'Cancel',
        style: 'cancel',
      },
